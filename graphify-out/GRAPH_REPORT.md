...
## Corpus Check
- 34 files · ~13,909 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 406 nodes · 1002 edges · 13 communities (10 shown, 3 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 113 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `536a7eed`
...
3. `main()` - 35 edges
4. `run_local_simulation()` - 33 edges
5. `ContributionLedger` - 32 edges
6. `ScheduledNode` - 31 edges
7. `MeshMessage` - 30 edges
8. `CliTests` - 30 edges
...
Cohesion: 0.10
Nodes (20): AetherMesh Core local prototype package., MessageLogPersistenceError, JSON-backed local message log persistence for batch simulations., Raised when a local message log JSON file cannot be safely saved., Write a local message log via temp-file then atomic replace., _remove_temp_file(), write_message_log(), Local mesh message envelopes for deterministic simulation output. (+12 more)
...
Cohesion: 0.07
Nodes (25): LocalMessageBus, MessageDelivery, Synchronous in-memory message bus for local AetherMesh simulation., A message accepted by the local bus with its deterministic sequence., Dependency-free local-only message bus for deterministic simulations., Register a node or reserved local service actor with the bus., Accept a message from a registered sender to a registered recipient., Return a copy of the ordered message log. (+17 more)

### Community 6 - "Community 6"
Cohesion: 0.11
Nodes (24): Run a local simulation from a validated JSON manifest., run_local_batch(), load_job_manifest(), LocalJobBatch, ManifestError, _parse_capabilities(), _parse_job_entry(), _parse_jobs() (+16 more)

### Community 8 - "Community 8"
Cohesion: 0.10
Nodes (7): build_parser(), main(), Command-line interface for the local AetherMesh prototype., Load an existing ledger and return read-only aggregate totals., summarize_ledger(), ArgumentParser, CliTests
...
- **Why does `Job` connect `Community 3` to `Community 0`, `Community 1`, `Community 5`, `Community 6`, `Community 12`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 8` to `Community 3`, `Community 12`, `Community 6`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `LocalSimulationResult` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 6`, `Community 12`?**
