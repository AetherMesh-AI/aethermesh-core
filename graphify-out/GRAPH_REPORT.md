...
Cohesion: 0.11
Nodes (13): NodeIdentity, Identity for one local node., LocalRunner, Run local work with optional, local-only operator safety controls.      Without, Execute supported local job types for a node., _run_in_local_process(), run_local_job(), _stopped_result() (+5 more)

### Community 1 - "Community 1"
Cohesion: 0.21
Nodes (17): _attribution(), _choice(), _details(), _evidence(), _identifier(), _identifier_list(), JobFailureSchemaError, _links() (+9 more)
...
Cohesion: 0.13
Nodes (9): JobResult, Structured result emitted after local job execution., _validation_summary(), _invalid(), Deterministic outcome from validating one reported job result., Validate a reported result against the assigned local job.      The current prot, validate_job_result(), ValidationResult (+1 more)
...
Cohesion: 0.06
Nodes (29): announce_local_node(), build_node_announcement_message_log_document(), build_node_heartbeat_message(), NodeAnnouncementError, normalize_announcement_capabilities(), Local-only node heartbeat announcement helpers., Build and write one local node heartbeat announcement message log., Raised when a local node announcement cannot be built or written safely. (+21 more)
...
Cohesion: 0.17
Nodes (11): _fallback_job_id(), _job_from_assignment_payload(), LocalNodeService, Synchronous local-only handler for one node's assigned-work inbox., Process unhandled ``job_assigned`` messages addressed to this node.          The, _coerce_job(), NoAvailableNodesError, Raised when local job assignment has jobs but no available nodes. (+3 more)
...
Cohesion: 0.08
Nodes (27): _accounted_units(), ContributionLedger, ContributionRecord, LedgerPersistenceError, load_existing_ledger_document(), load_ledger_document(), Contribution ledger helpers for local job results., Small in-memory ledger for prototype contribution accounting. (+19 more)
...
Cohesion: 0.40
Nodes (4): Failure types and examples, Phase 1 Job Failure Schema, Record contract, Redaction and storage rules
...
Cohesion: 0.21
Nodes (5): _canonical_json_bytes(), Canonical SHA-256 hashing for accounted local job results., Hash the canonical result fields used by result messages and audits., result_hash_from_fields(), ResultHashTests
