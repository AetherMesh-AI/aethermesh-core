...
## Corpus Check
- 136 files · ~180,996 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2198 nodes · 5922 edges · 112 communities (94 shown, 18 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 399 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `66e734d3`
...
Cohesion: 0.18
Nodes (9): dispatch_local_batch_command(), Dispatch a manifest batch to a local message log without execution., build_dispatch_message_log_document(), Build a deterministic version 1 assignment-only dispatch document., JobAssignment, Structured local assignment of one job to one node., Serialize the assignment into a JSON-compatible dictionary., Assign jobs to available capable nodes with deterministic fair ordering. (+1 more)
...
Cohesion: 0.12
Nodes (28): CapabilityRecordError, Validation for the local-only version 1 capability record contract., Raised when a local capability record is incomplete or dishonest., Validate and return one local-only capability record without writing it.      A, _reject_unknown_fields(), _require_attribution(), _require_identifier(), _require_identifier_list() (+20 more)
...
Cohesion: 0.13
Nodes (13): InboxReplayRequest, dispatch_local_batch(), LocalDispatchResult, _node_heartbeat_payloads(), Structured result for local assignment-only dispatch., Build a local dispatch log with heartbeats and job assignments only.      This f, _coerce_node(), LocalScheduler (+5 more)
...
Cohesion: 0.13
Nodes (10): Run the fixed local simulation demo used by the CLI command., run_default_local_simulation(), build_message_log_document(), Build a deterministic version 1 audit document for local mesh messages., LocalSimulationResult, Structured, deterministic output from a local multi-node simulation., Run local jobs across local node identities using scheduler assignment.      Thi, run_local_simulation() (+2 more)
...
Cohesion: 0.18
Nodes (22): _assert_assignment_matches_receipt(), _assert_contribution_message_matches_receipt(), _assert_equal(), _assert_message_order(), _assert_result_message_matches_receipt(), _assert_validation_message_matches_receipt(), audit_local_flow(), _find_flow_message_for_receipt() (+14 more)
...
Cohesion: 0.14
Nodes (14): _fallback_job_id(), InboxProcessResult, _job_from_assignment_payload(), LocalNodeService, Structured result returned by one local inbox processing pass., Synchronous local-only handler for one node's assigned-work inbox., Process unhandled ``job_assigned`` messages addressed to this node.          The, _coerce_job() (+6 more)

### Community 47 - "Community 47"
Cohesion: 0.13
Nodes (20): dispatch_peer_batch_command(), Run a local simulation from a validated JSON manifest., Dispatch manifest jobs to heartbeat-derived peers without execution., run_local_batch(), load_job_manifest(), _load_manifest_document(), load_manifest_jobs(), LocalJobBatch (+12 more)

### Community 48 - "models.py"
Cohesion: 0.10
Nodes (19): _log_ephemeral_identity_active(), _mark_ephemeral_artifact(), Command-line interface for the local AetherMesh prototype., _use_ephemeral_identity(), Assignment-only local dispatch for manifest-backed batches., AetherMesh Core local prototype package., MessageDelivery, Synchronous in-memory message bus for local AetherMesh simulation. (+11 more)
...
Cohesion: 0.14
Nodes (12): NodeRuntimeService, Raised when local runtime state cannot be safely loaded or written., Raised when a requested local validation receipt is not stored., Run a queued local submission; this is not a daemon or remote boundary., Central service for local node config, lifecycle status, peers, and jobs., Return read-only, redacted summaries of locally registered experts., Read one submission and its optional local execution evidence., Read derived local job audit events without mutating stored evidence. (+4 more)

### Community 54 - "Community 54"
Cohesion: 0.11
Nodes (35): _append_identity_reset_receipt(), _contains_secret_identity_fragment(), _gpu_input(), _identity_artifact_has_identity_metadata(), _identity_artifact_mentions_node(), _identity_document_creator_node_id(), _identity_reset_artifact_ref(), _identity_reset_warning() (+27 more)

### Community 55 - "Community 55"
Cohesion: 0.13
Nodes (10): _accounted_units(), Contribution ledger helpers for local job results., Record one result and return its local contribution record.          Only comple, _remove_temp_file(), _string_output(), _canonical_json_bytes(), Canonical SHA-256 hashing for accounted local job results., Hash the canonical result fields used by result messages and audits. (+2 more)

### Community 56 - "Community 56"
Cohesion: 0.13
Nodes (12): ContributionLedger, load_existing_ledger_document(), load_ledger_document(), Small in-memory ledger for prototype contribution accounting., Serialize the ledger into the small local JSON file shape., Load a JSON-backed local ledger, treating a missing file as empty.      The retu, Load an existing JSON-backed local ledger without creating defaults., Write a JSON-backed local ledger via temp-file then atomic replace. (+4 more)
...
Cohesion: 0.37
Nodes (13): _duplicate_flow_message(), _find_message(), FlowAuditTamperTests, _load_json(), _sample_receipt(), _set_dispatch_assignment_payload(), _set_flow_message_payload(), _set_flow_message_sender() (+5 more)
...
Cohesion: 0.14
Nodes (14): _emitted_messages_from_inbox_result(), _inbox_process_result_to_dict(), _mark_ephemeral_message_log(), _node_artifact_filename(), _node_artifact_path(), _node_ids_from_replayed_messages(), process_local_inbox(), Return a deterministic, non-merging filename for one manifest node id. (+6 more)
...
Cohesion: 0.42
Nodes (5): Run dispatch and all available local worker inboxes as one local flow., run_local_flow(), _artifact_contents(), FlowAuditTests, _write_manifest()

### Community 104 - "Community 104"
Cohesion: 0.28
Nodes (8): _create_identity_document_without_overwrite(), Create an identity JSON file atomically without replacing a winner., atomic_create_json(), _publish_json(), Shared JSON file persistence helpers for local-only artifacts., Create one JSON document atomically without replacing an existing file., Best-effort removal for abandoned atomic-write temp files., remove_temp_file()
...
- **Why does `Job` connect `Community 3` to `Community 32`, `Community 0`, `Community 66`, `Community 99`, `Community 5`, `Community 6`, `Community 71`, `Community 46`, `Community 47`, `Community 16`, `models.py`, `Community 53`, `Community 22`, `Community 24`, `Community 27`, `Community 95`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 8` to `Community 1`, `Community 2`, `Community 6`, `Community 10`, `Community 14`, `Community 24`, `Community 25`, `Community 27`, `Community 28`, `Community 34`, `Community 35`, `Community 37`, `Community 44`, `Community 45`, `Community 47`, `models.py`, `Community 50`, `Community 95`, `Community 103`, `Community 107`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `MeshMessage` connect `Community 57` to `Community 32`, `Community 1`, `Community 2`, `Community 5`, `Community 6`, `Community 103`, `Community 8`, `Community 43`, `Community 76`, `Community 46`, `models.py`, `Community 16`, `Community 22`, `Community 24`, `Community 89`, `Community 28`, `Community 25`, `Community 95`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
