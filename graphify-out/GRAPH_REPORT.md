...
## Corpus Check
- 132 files · ~177,603 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2141 nodes · 5776 edges · 104 communities (89 shown, 15 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 396 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fb4cd223`
...
Cohesion: 0.12
Nodes (30): collect_local_outboxes(), _load_inbox_document(), load_local_inbox(), _load_outbox_document(), local_inbox_path(), local_outbox_path(), LocalTransportError, _message_from_inbox_entry() (+22 more)
...
Cohesion: 0.13
Nodes (21): _emitted_messages_from_inbox_result(), _inbox_process_result_to_dict(), InboxReplayRequest, _log_ephemeral_identity_active(), _mark_ephemeral_artifact(), _mark_ephemeral_message_log(), _node_artifact_filename(), _node_artifact_path() (+13 more)
...
Cohesion: 0.11
Nodes (32): _bytes_to_gb(), collect_hardware_identity_inputs(), _colon_value(), _count_display_chips(), _csv_first_value(), _darwin_hardware_inputs(), _default_goos(), HardwareIdentityInputs (+24 more)
...
Cohesion: 0.13
Nodes (34): ArgumentParser, CompletedProcess, Namespace, changed_pyproject_from_base(), command_artifact_provenance(), command_dependency_audit(), command_dependency_review(), command_mutation_score() (+26 more)
...
Cohesion: 0.14
Nodes (8): Run the fixed local simulation demo used by the CLI command., run_default_local_simulation(), build_message_log_document(), Build a deterministic version 1 audit document for local mesh messages., Run local jobs across local node identities using scheduler assignment.      Thi, run_local_simulation(), _summary_to_simulation_dict(), LocalSimulationTests
...
Cohesion: 0.22
Nodes (7): build_dispatch_message_log_document(), Build a deterministic version 1 assignment-only dispatch document., JobAssignment, Structured local assignment of one job to one node., Serialize the assignment into a JSON-compatible dictionary., Assign jobs to available capable nodes with deterministic fair ordering., _node_roster_entry()
...
Cohesion: 0.12
Nodes (24): build_receipt_document(), _json_compatible_dict(), _json_compatible_list(), _json_compatible_value(), load_receipt_document_if_exists(), _output_summary(), Deterministic local work receipt documents for processed jobs., Write a receipt document via temp-file then atomic replace. (+16 more)
...
Cohesion: 0.14
Nodes (15): _fallback_job_id(), _job_from_assignment_payload(), LocalNodeService, ProcessedAssignment, Deterministic audit data for one inbox assignment processed locally., Synchronous local-only handler for one node's assigned-work inbox., Process unhandled ``job_assigned`` messages addressed to this node.          The, _single_emitted_message() (+7 more)
...
Cohesion: 0.15
Nodes (13): AetherMesh Core local prototype package., Contribution ledger helpers for local job results., MessageDelivery, Synchronous in-memory message bus for local AetherMesh simulation., A message accepted by the local bus with its deterministic sequence., Core data models for the local AetherMesh prototype., Local-only node inbox processing service for assigned work messages., Canonical SHA-256 hashing for accounted local job results. (+5 more)
...
Cohesion: 0.67
Nodes (3): NoAvailableNodesError, Raised when local job assignment has jobs but no available nodes., ValueError
...
Cohesion: 0.12
Nodes (13): NodeRuntimeService, Raised when a requested local validation receipt is not stored., Run a queued local submission; this is not a daemon or remote boundary., Central service for local node config, lifecycle status, peers, and jobs., Return read-only, redacted summaries of locally registered experts., Accept one local job request without dispatching or validating it., Read one submission and its optional local execution evidence., Read derived local job audit events without mutating stored evidence. (+5 more)

### Community 54 - "Community 54"
Cohesion: 0.11
Nodes (38): _contains_secret_identity_fragment(), _darwin_physical_mac_addresses(), _extract_labeled_value(), _extract_mac_addresses(), _identity_artifact_has_identity_metadata(), _identity_artifact_mentions_node(), _identity_document_creator_node_id(), IdentityPersistenceError (+30 more)
...
Cohesion: 0.12
Nodes (12): build_collected_outbox_message_log_document(), build_flow_message_log_document(), _message_to_document_entry(), Build a deterministic version 1 run-level local flow message log., Build a deterministic version 1 log from collected local transport outboxes., MeshMessage, message_from_mapping(), JSON-compatible message envelope for local mesh communication records. (+4 more)

### Community 58 - "release_update.py"
Cohesion: 0.13
Nodes (19): Serialize the result into a JSON-compatible dictionary., _config_api_host(), _config_api_port(), _config_enabled_work_types(), _config_identity_path(), _config_identity_persistence_enabled(), _config_node_id(), _config_node_name() (+11 more)
...
Cohesion: 0.12
Nodes (17): AEF: Aether Expert Fabric, AER: Adaptive Expert Routing, AetherMesh Core Architecture, Aggregator, Contribution tracking direction, Core responsibilities, Current prototype layer, Decision rule for new work (+9 more)
...
Cohesion: 0.22
Nodes (6): `GET /api/contributions` — contribution lookup, `GET /api/validation-receipts` — validation result lookup, Local node registration/startup, Migration note: schema contract version 1, Phase 1 API Schema Contract, `POST /api/jobs` — local work submission
...
Cohesion: 0.47
Nodes (3): materialize_local_inboxes(), Materialize addressed messages from a message log into per-node inbox files., _message_entry()
...
Cohesion: 0.16
Nodes (9): _default_home(), _memory_total_bytes(), _package_version(), _pid_is_alive(), Reusable local runtime service for AetherMesh frontends.  The CLI, local API, an, Filesystem paths used by one local AetherMesh runtime., Return installed package metadata for launchers and local dashboards., RuntimePaths (+1 more)
...
- **Why does `Job` connect `Community 3` to `Community 0`, `Community 32`, `Community 66`, `Community 98`, `Community 5`, `Community 6`, `Community 101`, `Community 46`, `Community 47`, `Community 16`, `models.py`, `Community 53`, `Community 22`, `Community 24`, `Community 27`, `Community 28`, `Community 29`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 8` to `Community 1`, `Community 34`, `Community 35`, `Community 2`, `Community 6`, `Community 10`, `Community 44`, `Community 45`, `Community 14`, `Community 47`, `Community 50`, `Community 20`, `Community 87`, `Community 24`, `Community 25`, `Community 27`, `Community 29`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `MeshMessage` connect `Community 57` to `Community 32`, `Community 1`, `Community 2`, `Community 98`, `Community 5`, `Community 6`, `Community 8`, `Community 43`, `Community 13`, `Community 14`, `Community 46`, `models.py`, `Community 16`, `Community 25`, `Community 28`, `Community 29`?**
