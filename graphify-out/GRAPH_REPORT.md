...
## Corpus Check
- 32 files · ~12,445 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 375 nodes · 909 edges · 14 communities (11 shown, 3 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 107 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b5040eff`
...
5. `run_local_simulation()` - 31 edges
6. `MeshMessage` - 28 edges
7. `CliTests` - 27 edges
8. `LocalMessageBus` - 26 edges
9. `NodeIdentity` - 25 edges
10. `NodeRegistry` - 25 edges
...
Cohesion: 0.11
Nodes (23): Run the fixed local simulation demo used by the CLI command., run_default_local_simulation(), Job, JobResult, A small in-memory job assigned to a local node., Structured result emitted after local job execution., build_text_stats_output(), Run one local job and return a structured result. (+15 more)
...
Cohesion: 0.07
Nodes (24): LocalMessageBus, Dependency-free local-only message bus for deterministic simulations., Register a node or reserved local service actor with the bus., Accept a message from a registered sender to a registered recipient., Return a copy of the ordered message log., Return a copy of the deterministic inbox for a registered node., MeshMessage, JSON-compatible message envelope for local mesh communication records. (+16 more)

### Community 6 - "Community 6"
Cohesion: 0.11
Nodes (20): Run a local simulation from a validated JSON manifest., run_local_batch(), load_job_manifest(), LocalJobBatch, ManifestError, _parse_job_entry(), _parse_jobs(), _parse_node_entry() (+12 more)

### Community 7 - "Community 7"
Cohesion: 0.33
Nodes (7): _require_non_empty_string(), _require_supported_message_type(), _validate_json_compatible(), NoAvailableNodesError, Raised when local job assignment has jobs but no available nodes., Assign job IDs round-robin across available nodes in input order., ValueError
...
- **Why does `run_local_simulation()` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 5`, `Community 6`, `Community 7`, `Community 12`?**
  _High betweenness centrality (0.139) - this node is a cross-community bridge._
- **Why does `LocalSimulationResult` connect `Community 12` to `Community 0`, `Community 1`, `Community 2`, `Community 3`, `Community 5`, `Community 6`?**
