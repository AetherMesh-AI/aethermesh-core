...
## Corpus Check
- 32 files · ~11,866 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 369 nodes · 890 edges · 13 communities (10 shown, 3 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 105 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `52354a5d`
...
1. `Job` - 47 edges
2. `JobResult` - 37 edges
3. `main()` - 32 edges
4. `ContributionLedger` - 31 edges
...
Cohesion: 0.16
Nodes (12): Command-line interface for the local AetherMesh prototype., AetherMesh Core local prototype package., Local mesh message envelopes for deterministic simulation output., _require_non_empty_string(), _require_supported_message_type(), _validate_json_compatible(), Core data models for the local AetherMesh prototype., Local-only node inbox processing service for assigned work messages. (+4 more)
...
Cohesion: 0.11
Nodes (22): Run the fixed local simulation demo used by the CLI command., run_default_local_simulation(), Job, JobResult, A small in-memory job assigned to a local node., Structured result emitted after local job execution., Serialize the result into a JSON-compatible dictionary., build_text_stats_output() (+14 more)
