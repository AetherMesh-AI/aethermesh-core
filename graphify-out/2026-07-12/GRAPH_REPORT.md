...
## Corpus Check
- 151 files · ~190,601 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2445 nodes · 6444 edges · 117 communities (103 shown, 14 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 413 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `207da62a`
...
Cohesion: 0.14
Nodes (22): collect_local_outboxes(), _load_inbox_document(), _load_outbox_document(), local_outbox_path(), LocalTransportError, _message_from_inbox_entry(), _message_from_outbox_entry(), _messages_from_outbox_document() (+14 more)
...
Cohesion: 0.07
Nodes (60): _accepted_result(), aggregate_local_flow(), AggregationError, build_local_flow_aggregate(), _is_accepted(), Deterministic local aggregation for completed flow artifact directories., Raised when a local flow aggregate cannot be built or written safely., Build a deterministic aggregate document after auditing a flow directory. (+52 more)
...
Cohesion: 0.14
Nodes (33): CompletedProcess, Namespace, changed_pyproject_from_base(), command_artifact_provenance(), command_dependency_audit(), command_dependency_review(), command_mutation_score(), command_pr_size() (+25 more)

### Community 18 - "Community 18"
Cohesion: 0.11
Nodes (31): configured_runtime_path(), default_local_runtime_config(), load_local_runtime_config(), load_optional_local_runtime_config(), load_or_create_local_runtime_config(), LocalRuntimeConfig, LocalRuntimeConfigError, parse_local_runtime_config() (+23 more)

### Community 19 - "Community 19"
Cohesion: 0.12
Nodes (12): NodeRegistry, In-memory source of truth for local simulation node state.      The registry is, Register a node ID or ScheduledNode in deterministic insertion order., Mark a known node available for future scheduler exports., Mark a known node offline for future scheduler exports., Record one deterministic local heartbeat for a known node., Return JSON-compatible roster entries in registration order., _RegistryNode (+4 more)
...
Cohesion: 0.15
Nodes (6): LocalScheduler, NoAvailableNodesError, Raised when local job assignment has jobs but no available nodes., In-memory deterministic scheduler for local prototype jobs., Assign jobs to available capable nodes with deterministic fair ordering., LocalSchedulerTests
...
Cohesion: 0.19
Nodes (10): Run the fixed local simulation demo used by the CLI command., run_default_local_simulation(), build_message_log_document(), Build a deterministic version 1 audit document for local mesh messages., Job, A small in-memory job assigned to a local node., Run local jobs across local node identities using scheduler assignment.      Thi, run_local_simulation() (+2 more)
...
Cohesion: 0.16
Nodes (9): _accounted_units(), Record one result and return its local contribution record.          Only comple, _string_output(), _fallback_job_id(), _job_from_assignment_payload(), _coerce_job(), Minimal scheduler view of a job., ScheduledJob (+1 more)

### Community 31 - "Community 31"
Cohesion: 0.12
Nodes (14): InboxReplayRequest, empty_node_processing_state(), load_node_processing_state(), LocalNodeProcessingState, NodeStatePersistenceError, Versioned local node processing state persistence., Write a local node-state JSON document via temp-file then atomic replace., Raised when a local node-state JSON file cannot be safely loaded or saved. (+6 more)
...
Cohesion: 0.14
Nodes (12): dispatch_local_batch(), LocalDispatchResult, _node_heartbeat_payloads(), Structured result for local assignment-only dispatch., Serialize a deterministic, intentionally small CLI summary., Build a local dispatch log with heartbeats and job assignments only.      This f, Return scheduler-compatible nodes in registration order., _coerce_node() (+4 more)
...
Cohesion: 0.13
Nodes (16): Assignment-only local dispatch for manifest-backed batches., AetherMesh Core local prototype package., Contribution ledger helpers for local job results., MessageDelivery, Synchronous in-memory message bus for local AetherMesh simulation., A message accepted by the local bus with its deterministic sequence., JSON-backed local message log persistence for batch simulations., Local mesh message envelopes for deterministic simulation output. (+8 more)
...
Cohesion: 0.09
Nodes (21): ContributionLedger, ContributionRecord, LedgerPersistenceError, load_existing_ledger_document(), load_ledger_document(), Small in-memory ledger for prototype contribution accounting., Audit record derived from one local job result., Serialize the ledger into the small local JSON file shape. (+13 more)

### Community 57 - "Community 57"
Cohesion: 0.16
Nodes (16): _emitted_messages_from_inbox_result(), _inbox_process_result_to_dict(), _log_ephemeral_identity_active(), _mark_ephemeral_artifact(), _mark_ephemeral_message_log(), _node_artifact_filename(), _node_artifact_path(), _node_ids_from_replayed_messages() (+8 more)

### Community 58 - "release_update.py"
Cohesion: 0.07
Nodes (33): _availability(), _capability_check_payload(), _capability_manifest_id(), _capability_provenance(), _config_api_host(), _config_api_port(), _config_capability_resource_hints(), _config_enabled_work_types() (+25 more)
...
Cohesion: 0.08
Nodes (6): create_app(), Create the localhost API/dashboard app used by CLI and UI frontends., ApiErrorTests, CapabilityRecordInspectionTests, ModelManifestInspectionTests, RuntimeServiceTests
...
Cohesion: 0.17
Nodes (15): Load an existing ledger and return read-only aggregate totals., _require_int_result_field(), _run_local_flow_with_roster(), summarize_ledger(), load_local_inbox(), local_inbox_path(), materialize_local_inboxes(), Load and validate only ``node_id``'s local transport inbox messages. (+7 more)
...
Cohesion: 0.07
Nodes (40): ProcessedAssignment, Deterministic audit data for one inbox assignment processed locally., build_receipt_document(), _json_compatible_dict(), _json_compatible_list(), _json_compatible_value(), load_receipt_document_if_exists(), _output_summary() (+32 more)
...
Cohesion: 0.12
Nodes (14): _error_response(), _lifespan(), Local FastAPI app for the AetherMesh node dashboard., Return the stable, deliberately non-provenance API error envelope., Classify expected local runtime failures without exposing their text., _request_id(), _runtime_error_code(), FastAPI (+6 more)
...
Cohesion: 0.22
Nodes (14): _contribution(), _exact_integer(), _failure_reasons(), _identifier(), JobResultSchemaError, _lineage(), _object(), Validation for local, auditable Phase 1 job-result records. (+6 more)
...
Cohesion: 0.15
Nodes (11): LocalNodeService, Synchronous local-only handler for one node's assigned-work inbox., Process unhandled ``job_assigned`` messages addressed to this node.          The, LocalSimulationResult, Structured, deterministic output from a local multi-node simulation., Serialize the simulation result into a JSON-compatible dictionary.          Vali, _validation_summary(), _invalid() (+3 more)
...
Cohesion: 0.10
Nodes (19): build_keyword_extract_output(), build_text_chunk_output(), build_text_embed_output(), build_text_retrieve_output(), build_text_stats_output(), _preferred_text_chunk_split(), Local in-memory runner for the first AetherMesh executable slice., Run local work with optional, local-only operator safety controls.      Without (+11 more)
...
## Knowledge Gaps
- **322 isolated node(s):** `name`, `version`, `private`, `main`, `{ spawnSync }` (+317 more)
