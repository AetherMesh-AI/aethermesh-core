...
Accepted payload normalization is deterministic: the validated input payload is preserved as submitted and its canonical sorted-key UTF-8 JSON is hashed; safe lineage references and attribution metadata are preserved in caller order and shape after validation. Runtime-owned IDs, timestamps, manifest references, worker identity, validation status, and validated contribution units are added by the receiving node rather than accepted from attribution metadata. These rules allow local validation receipts to reproduce the input hash and attribution boundary without remote consensus.
...
## Local Validation Receipt v5

### Observed execution timing

A receipt created by the local executor includes `executor_started_at`, `executor_finished_at`, and `duration_ms` when both execution boundary timestamps were captured during that actual run. `duration_ms` is the exact millisecond difference between those recorded UTC timestamps and is checked against the linked result record before export. Existing or otherwise timing-unavailable receipts omit `duration_ms`; the service never estimates or backfills it. This optional metadata supplements, rather than changes, the receipt's creator ID, manifest reference, work ID, lineage, validation result, and contribution attribution.

`GET /api/validation-receipts` is read-only. Without selectors it returns Local Validation Receipt List v1: `schema_version` (`1`), `network_mode`, `total`, and `validation_receipts`. Every list item preserves `receipt_id`, `validation_receipt_id`, `work_id`, `creator_node_id`, `manifest_ref`, `lineage_parent_ids`, `contribution_attribution`, `status`, `validation_status`, `validated_at`, and `validation_summary`. It exposes no filesystem paths other than the receipt's persisted relative manifest reference and does not create, recompute, or modify validation outcomes. `GET /api/validation-receipts/{receipt_id}` returns the full Local Validation Receipt v5 described below. The legacy query detail lookup remains available for compatibility.

Every detail receipt includes `schema_version` (`5`), machine-readable `status` (`accepted` or `rejected`), `rejection_reason` (null only when accepted), `next_local_action`, `receipt_id`, `validation_receipt_id`, `work_id`, `creator_node_id`, `model_expert_id`, `requester_identity`, `manifest_ref`, `input_payload_hash`, `lineage_parent_ids`, `validation_status`, `validation_method`, `validated_at`, `validator_identity`, `validator_software`, `contribution_attribution`, `validation_scope`, `validation` (including `job_id`), and `evidence`. `model_expert_id` is a stable, deterministic, non-secret, whitespace-free local ID for the model, expert, or worker that produced the linked output; it is distinct from node and contribution identities. `validator_software` records the validator implementation version, optional build identity, runtime, platform, and matching receipt schema version at receipt creation time without local paths. `accepted` means required local validation passed; it is not network consensus. `validated_at` is recorded locally after validation completes as a UTC ISO 8601 timestamp ending in `Z`; it is audit timing, not consensus time. `next_local_action` is a concrete local replay, retention, inspection, or correction action; it is required even for rejected receipts so a reason cannot create false confidence. `validation_method` identifies the concrete local check and repeats the receipt's manifest, creator, work, lineage, and contribution provenance so exports remain self-describing. `validation_receipt_id` is the stable, unique local receipt identifier and matches the legacy lookup-compatible `receipt_id`. The payload hash must match the referenced manifest's canonical input payload. A missing receipt is rejected with 404; malformed lookup combinations are rejected with 400.
...
```json
{
  ...
  "validator_software": {
    ...
    "validator_version": "0.2.0-alpha",
    "validator_build_identifier": "unknown",
