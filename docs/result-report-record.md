...
`POST /api/result-reports/preflight` and `NodeRuntimeService.preflight_local_result_report()` provide the local-only candidate boundary before validation. They require the version 8 schema plus an existing matching local manifest, creator, capability, reserved receipt ID, and lineage. Candidate reports must remain `pending` or `not_run` with `result_hash: null`; preflight does not create a result record, receipt, contribution, or lineage state. A malformed candidate produces a bounded local `data/rejected-result-reports/` audit record containing its machine-readable reason, processing timestamp, and original report (or a hash and bounded prefix when it exceeds 16 KiB).
