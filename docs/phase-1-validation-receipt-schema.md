...
| `manifest_reference` | `null` when no local reference applies, otherwise an exact local `manifest_id` and safe repository-relative `manifest_ref`. |
