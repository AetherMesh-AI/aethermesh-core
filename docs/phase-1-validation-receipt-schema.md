...
All `contribution` fields are required. `submitter_id`, `local_node_id`, `claimed_role`, and `contribution_manifest_ref` are nullable placeholders when the local input did not provide the corresponding attribution. A non-null contribution manifest reference must be a safe repository-relative local path. These fields record a claim only; they do not calculate credit or rewards.
