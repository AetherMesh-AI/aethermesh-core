...
Every top-level field is required and unknown top-level fields are rejected. This strict rule prevents a reader from silently accepting an unreviewed critical field. Version 10 requires:
...
| `job_id` | Required local job identifier assigned before execution; it must equal `work_id` in schema version 10. |
| `work_id` | Work item validated by the receipt. |
| `manifest_id` | Content-addressed source work manifest identifier, or `null` when no manifest defined the validated work. |
| `manifest_reference` | `null` when no manifest defined the work, otherwise the exact local `manifest_id` and a safe repository-relative `manifest_ref`. |
...
`manifest_reference` binds an applicable receipt to a safe repository-relative manifest path and its content-addressed ID. Consumers resolve the path relative to their local receipt root and recompute the manifest hash before relying on it. It is local provenance evidence only, not a network availability or consensus claim. `validation_method` has exactly `kind`, `description`, `manifest_id`, `creator_node_id`, `contributor_node_id`, `work_id`, `lineage_parent_work_ids`, and `contribution_manifest_ref`. `kind` names a concrete local check such as `deterministic_fixture_replay`, `manifest_comparison`, `schema_validation`, or `hash_verification`; `description` explains what was checked. Its manifest, creator, contributor, work, lineage, and contribution references must exactly match their receipt fields. This makes a stored or exported receipt self-describing without changing attribution.

Version 10 supersedes version 9 by requiring `manifest_reference`: it is `null` when no manifest defined the work, otherwise it contains the matching content-addressed manifest ID and a safe local reference. This makes applicable manifest provenance locally resolvable without inventing a manifest for standalone validation. Existing version 9 receipts remain identifiable as version 9 records and must be migrated explicitly rather than silently reinterpreted. Version 7 superseded version 6 by requiring non-empty `contributor_node_id` at the receipt, validation-method, lineage, and contribution-manifest attribution points; those values must match. Version 6 superseded version 5 by requiring `validator_software`. Version 5 superseded version 4 by requiring `status` and `rejection_reason`. Version 4 superseded version 3 by requiring `validated_at`, a canonical UTC ISO 8601 completion timestamp. Version 3 superseded version 2 by requiring `validation_method`. Version 2 superseded version 1 by requiring the algorithm-prefixed `result_hash` format.
