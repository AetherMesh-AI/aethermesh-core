...
All `contribution` fields are required. `submitter_id`, `local_node_id`, `claimed_role`, and `contribution_manifest_ref` are nullable placeholders when the local input did not provide the corresponding attribution. They record a claim only; they do not calculate credit or rewards.

All `evidence` fields are required. `test_command`, `environment_summary`, `log_path`, and `artifact_path` are nullable when unavailable. `exit_code` is nullable when no command was run. `reason` is a required deterministic local pass/fail/error/skip explanation; it may be null only when no deterministic explanation exists yet. Paths are portable local references, never machine-absolute paths.

## Stable hash and replay expectation

Receipts are plain JSON and use compact UTF-8 JSON with sorted keys for `receipt_hash`. The hash deliberately excludes `created_at` and `receipt_hash`, so repeated local validation of identical work, lineage, attribution, and evidence produces the same receipt hash even when audit timestamps differ. `receipt_id` is supplied by the stable local work identity and must likewise repeat for the same work. Future replay code should validate the schema and recompute `receipt_hash` before using the evidence.
