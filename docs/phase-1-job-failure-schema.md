...
| `details` | Required nullable slots for `exit_code`, `signal`, `timeout_ms`, `validation_error_code`, `missing_input_id`, `manifest_mismatch`, `receipt_mismatch`, and `environment_issue_code`. Values are codes or references, never payload contents. |
