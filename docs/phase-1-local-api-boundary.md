...
`POST /api/jobs` is a localhost-only submission route backed by `NodeRuntimeService.submit_local_job(request)`. Its request is a JSON object with non-empty `job_type`, object `payload`, non-empty `creator_node_id`, non-empty `requested_validation_mode`, required `lineage_parent_refs` (a list of non-empty strings, which may be empty for root work), and required object `attribution_metadata` (which may be empty). It writes one immutable `data/job-submissions/<job-id>.json` manifest only after validation and returns `job_id`, `status: accepted_pending_execution`, `manifest_ref`, `next_validation_expectation: pending_requested_local_validation`, and `network_mode: local-only-no-p2p`. Submission does not dispatch work, create a receipt, complete validation, contact peers, or award credit. The versioned schemas and compatibility rule are in `docs/phase-1-api-schema-contract.md`.
