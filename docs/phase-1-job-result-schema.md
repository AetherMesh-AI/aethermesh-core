...
`aethermesh_core.job_result_schema.validate_job_result_document` defines the local-only version-9 record emitted after a job has executed. It is an audit record for the runnable prototype, not a peer protocol, consensus claim, dashboard model, or reward ledger. The existing in-memory `JobResult` runner value remains intentionally small; this record is its durable provenance contract.

Version 9 supersedes version 8 by requiring `model_expert_id` for every durable job output. Existing version 8 records are not version 9 records and must not be silently reinterpreted; migrate them explicitly before passing them to the version 9 validator.

## Required record fields

| Field | Source | Purpose |
| --- | --- | --- |
| `schema_version` | locally generated | Fixed integer `9`. |
...
| `model_expert_id` | local execution metadata | Required stable, deterministic, non-secret, whitespace-free identifier for the model, expert, or local worker that produced the result (for example, `local-runner:aethermesh-local-runner@1`). It is separate from `creator_node_id`, executor identity, and contribution attribution. It is local Phase 1 audit metadata, not an expert registry, model claim, or routing-consensus claim. |
