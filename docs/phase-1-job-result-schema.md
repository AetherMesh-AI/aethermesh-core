...
`aethermesh_core.job_result_schema.validate_job_result_document` defines the local-only version-6 record emitted after a job has executed. It is an audit record for the runnable prototype, not a peer protocol, consensus claim, dashboard model, or reward ledger. The existing in-memory `JobResult` runner value remains intentionally small; this record is its durable provenance contract.

Version 6 supersedes version 5 by requiring a local `reported_at` audit timestamp. Existing version 5 records are not version 6 records and must not be silently reinterpreted; migrate them explicitly before passing them to the version 6 validator.

## Required record fields

| Field | Source | Purpose |
| --- | --- | --- |
| `schema_version` | locally generated | Fixed integer `6`. |
