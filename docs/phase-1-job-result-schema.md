...
`aethermesh_core.job_result_schema.validate_job_result_document` defines the local-only version-5 record emitted after a job has executed. It is an audit record for the runnable prototype, not a peer protocol, consensus claim, dashboard model, or reward ledger. The existing in-memory `JobResult` runner value remains intentionally small; this record is its durable provenance contract.

Version 5 supersedes version 4 by requiring the canonical result hash in the report itself and rejecting stale hashes. Existing version 4 records are not version 5 records and must not be silently reinterpreted; migrate them explicitly before passing them to the version 5 validator.

## Required record fields

| Field | Source | Purpose |
| --- | --- | --- |
| `schema_version` | locally generated | Fixed integer `5`. |
| `result_hash` | canonical result payload | Required 64-character lowercase SHA-256 digest; schema validation recomputes it and rejects stale values. |
