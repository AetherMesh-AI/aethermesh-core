...
Verify an existing local flow artifact directory without rerunning work or rewriting artifacts:

```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -m aethermesh_core.cli verify-local-flow --output-dir /tmp/aethermesh-local-flow
```
